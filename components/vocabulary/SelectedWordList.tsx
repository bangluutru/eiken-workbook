...
function EditableWord({ vocab }: { vocab: Vocabulary }) {
      ...
      <div className="flex items-start justify-between gap-2 py-2 px-3 group">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {vocab.pos && <span className="text-xs text-gray-400">{vocab.pos}</span>}
            {vocab.level.map((l) => (
              <span key={l} className="text-xs px-1 py-0.5 bg-gray-100 text-gray-500 rounded">
                {l}
              </span>
            ))}
            {vocab.source === 'custom' && (
  ...
  return (
    <div className="py-2 px-3 bg-blue-50 space-y-2">
      <div className="font-semibold text-sm text-gray-900 mb-1">{vocab.word}</div>
      {(
        [
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button
          ...
          className="flex items-center gap-1 px-2 py-1 bg-white text-gray-600 text-xs rounded border border-gray-200 hover:bg-gray-50"
        >
          <X className="w-3 h-3" />
          {t(locale, 'cancel')}
        </button>
